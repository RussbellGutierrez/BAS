	...
	if ($deco != '0') {
		...
		while($datos = sqlsrv_fetch_array($execute)){
			if ($user == 0 && $coment == 0) {
				...
				$row = array('fecha'=>$datos['f_in'],'transcurrido'=>$diff,'usuario'=>$datos['usuario'],'nomusu'=>$datos['nomusu'],'codcoment'=>$codigo,'titulo'=>$datos['titulo'],'descrip'=>$datos['descrip'],'tipo'=>$datos['tipo'],'estado'=>$datos['estado'],'avance'=>$avance,'finalizado'=>$datos['finalizado'],'anulado'=>$datos['anulado'],'iniciado'=>$datos['iniciado'],'fecesti'=>$fecesti->format("Y-m-d"));
